        ...
        {page === "ctf" ? (
          ...
          <main className="section-page">
            ...
            <div className="policy-grid">
              <section className="page-card codex-session-card"><div className="page-card-heading"><span>CODEX SESSION</span><b>{codexSession.configured ? "CONFIGURED" : "NOT CONFIGURED"}</b></div><h3>Codex Session ID or bridge command</h3><input className="wide-input" aria-label="Codex Session ID or bridge command" onChange={(event) => setCodexSession((current) => ({ ...current, command: event.target.value }))} placeholder="Session UUID or /absolute/path/to/bridge" value={codexSession.command} /><p>Paste a Codex Session UUID to resume it through the local Codex app-server, or enter a JSON bridge command. It is stored locally and every change is audited.</p><button className="button primary" onClick={() => void saveCodexSession()} type="button">Save Codex Session</button></section>
